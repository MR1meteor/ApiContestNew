...
namespace ApiContestNew.Core.Models.Entities
{
    public class Area : BaseEntity
    {
        ...
        public bool IsValidWithoutId()
        {
            ...
            {
                for (int k = 3; k < AreaPoints.Count; k++)
                {
                    int j = i + 2;
                    int j1 = j;
                    int j2 = j + 1;

                    if (j2 >= AreaPoints.Count)
                    {
                        j2 = 0;
                    }

                    if (j1 >= AreaPoints.Count)
                    {
                        j2 = 1;
                        j1 = 0;
                    }
                    ...
                    if (AreLinesCrossing(
                        ...
                        ((List<LocationPoint>)AreaPoints)[j1],
                        ((List<LocationPoint>)AreaPoints)[j2]))
                    {
                        return false;
                    }
                }
        ...
        {
            double equationIndex1 = (p2.Longitude - p1.Longitude) / (p2.Latitude - p1.Latitude);
            double equationOffset1 = (p2.Latitude * p1.Longitude - p1.Latitude * p2.Longitude) / (p2.Latitude - p1.Latitude);

            double equationIndex2 = (p4.Longitude - p3.Longitude) / (p4.Latitude - p3.Latitude);
            double equationOffset2 = (p4.Latitude * p3.Longitude - p3.Latitude * p4.Longitude) / (p4.Latitude - p3.Latitude);
            ...
            try
            {
                ...
                if (
                    (p1.Latitude < crossingLatitude && p2.Latitude > crossingLatitude ||
                    p1.Latitude > crossingLatitude && p2.Latitude < crossingLatitude) &&
                    (p1.Longitude < crossingLongitude && p2.Longitude > crossingLongitude ||
                    p1.Longitude > crossingLongitude && p2.Longitude < crossingLongitude) &&
                    (p3.Latitude > crossingLatitude && p4.Latitude < crossingLatitude ||
                    p3.Latitude < crossingLatitude && p4.Latitude > crossingLatitude) &&
                    (p3.Longitude < crossingLongitude && p4.Longitude > crossingLongitude ||
                    p3.Longitude > crossingLongitude && p4.Longitude < crossingLongitude)
                    )
                {
                    return true;
